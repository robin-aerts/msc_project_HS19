...
def undersample(X_train, y_train, n_undersample = 1000, do_printouts = True):
...
#define model
def build_model(n_nodes, X_train, nlabel = 3):
...
def train_models(design_df, n_manholes):
  #add virtual manholes
  new_label = 'level_' + str(n_manholes)
  design_df[new_label] = design_df['level_3'] * np.random.normal(1, 0.1)


  #separate X and y, training and testing
  n_train = round(design_df.shape[0] * 0.8)
  
  X_train = design_df.iloc[0:n_train,:-n_manholes]
  y_train = design_df.iloc[0:n_train,-n_manholes:]
  X_test = design_df.iloc[n_train:,:-n_manholes]
  y_test = design_df.iloc[n_train:,-n_manholes:]
  ...
  model = build_model(n_nodes = 16, X_train = normed_X_train,
                      nlabel = n_manholes)
  #train the model
...
swmm = pd.read_csv('pyswmm_multi_var.csv')
...
n_manholes_arr = np.array([4, 5, 6, 7])

metrics_matrix = np.array([])

for n_manholes in n_manholes_arr:
  print(n_manholes)
  metrics_array = train_models(design_df, n_manholes)
  print(metrics_array)
  metrics_matrix = np.append(metrics_matrix, metrics_array)


#plot metrics for different training data sizes
metrics_matrix = metrics_matrix.reshape((len(n_manholes_arr),3)).transpose()
